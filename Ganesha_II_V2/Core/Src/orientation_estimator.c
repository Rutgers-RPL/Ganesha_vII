...
void orientation_estimator_reset_from_accel(struct Orientation_Estimator* estimator, float accel_x, float accel_y, float accel_z) {
	...
	float norm;
    arm_sqrt_f32(accel_x*accel_x + accel_y*accel_y + accel_z*accel_z, &norm);

    accel_norm[0] = accel[0] / norm;
    accel_norm[1] = accel[1] / norm;
    accel_norm[2] = accel[2] / norm;
    ...
    float dot;
    arm_dot_prod_f32(accel_norm, world, 3, &dot);
	...
	float axis_norm;
	arm_sqrt_f32(axis[0]*axis[0] + axis[1]*axis[1] + axis[2]*axis[2], &axis_norm);
