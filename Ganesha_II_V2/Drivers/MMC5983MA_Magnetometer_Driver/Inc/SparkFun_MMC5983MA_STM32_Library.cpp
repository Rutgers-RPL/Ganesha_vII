  ...
  httpswww.sparkfun.comproducts19034

  Do you like this library Help support open source hardware. Buy a board!

  Written by Ricardo Ramos  @ SparkFun Electronics, February 2nd, 2022.
  Modified for STM32 HAL compatibility.
  This file implements all functions used in the MMC5983MA High Performance Magnetometer STM32 Library.

  SparkFun code, firmware, and software is released under the MIT License(httpopensource.orglicensesMIT).
...
bool SFE_MMC5983MAsetShadowBit(uint8_t registerAddress, const uint8_t bitMask, bool doWrite)
{
...
bool SFE_MMC5983MAclearShadowBit(uint8_t registerAddress, const uint8_t bitMask, bool doWrite)
{
    uint8_t shadowRegister = nullptr;
...
bool SFE_MMC5983MAisShadowBitSet(uint8_t registerAddress, const uint8_t bitMask)
{
     Which register are we referring to
    switch (registerAddress)
    {
...
void SFE_MMC5983MAsetErrorCallback(void (_errorCallback)(SF_MMC5983MA_ERROR errorCode))
{
    errorCallback = _errorCallback;
}

const char SFE_MMC5983MAerrorCodeString(SF_MMC5983MA_ERROR errorCode)
{
  switch (errorCode)
  {
  case SF_MMC5983MA_ERRORNONE
    return NONE;
    break;
  case SF_MMC5983MA_ERRORI2C_INITIALIZATION_ERROR
    return I2C_INITIALIZATION_ERROR;
    break;
  case SF_MMC5983MA_ERRORSPI_INITIALIZATION_ERROR
    return SPI_INITIALIZATION_ERROR;
    break;
  case SF_MMC5983MA_ERRORINVALID_DEVICE
    return INVALID_DEVICE;
    break;
  case SF_MMC5983MA_ERRORBUS_ERROR
    return BUS_ERROR;
    break;
  case SF_MMC5983MA_ERRORINVALID_FILTER_BANDWIDTH
    return INVALID_FILTER_BANDWIDTH;
    break;    
  case SF_MMC5983MA_ERRORINVALID_CONTINUOUS_FREQUENCY
    return INVALID_CONTINUOUS_FREQUENCY;
    break;
  case SF_MMC5983MA_ERRORINVALID_PERIODIC_SAMPLES
...
bool SFE_MMC5983MAbegin(I2C_HandleTypeDef hi2c)
{
     Initializes I2C and check if device responds
    bool success = mmc_io.begin(hi2c);

    if (!success)
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORI2C_INITIALIZATION_ERROR);
        return false;
    }
    return isConnected();
}

bool SFE_MMC5983MAbegin(GPIO_TypeDef csPort, uint16_t csPin, SPI_HandleTypeDef hspi)
{
    bool success = mmc_io.begin(csPort, csPin, hspi);
    if (!success)
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORSPI_INITIALIZATION_ERROR);
        return false;
    }
    return isConnected();
}

bool SFE_MMC5983MAisConnected()
{
    ...
    if (!success)
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORBUS_ERROR);
        return false;
    }
    if (response != PROD_ID)
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORINVALID_DEVICE);
        return false;
    }
    return true;
}

int SFE_MMC5983MAgetTemperature()
{
    ...
    float temperature = -75.0f + (static_castfloat(result)  (200.0f  255.0f));

     Return the integer part of the temperature.
    return static_castint(temperature);
}

bool SFE_MMC5983MAsoftReset()
{
...
bool SFE_MMC5983MAenableInterrupt()
{
...
bool SFE_MMC5983MAdisableInterrupt()
{
...
bool SFE_MMC5983MAisInterruptEnabled()
{
...
bool SFE_MMC5983MAenable3WireSPI()
{
     This bit must be set through the shadow memory or we won't be
...
bool SFE_MMC5983MAdisable3WireSPI()
{
     This bit must be cleared through the shadow memory or we won't be
...
bool SFE_MMC5983MAis3WireSPIEnabled()
{
     Get the bit value from the shadow register since the IC does not
...
bool SFE_MMC5983MAperformSetOperation()
{
...
bool SFE_MMC5983MAperformResetOperation()
{
...
bool SFE_MMC5983MAenableAutomaticSetReset()
{
     This bit must be set through the shadow memory or we won't be
     able to check if automatic setreset is enabled using isAutomaticSetResetEnabled()
    return (setShadowBit(INT_CTRL_0_REG, AUTO_SR_EN));
}

bool SFE_MMC5983MAdisableAutomaticSetReset()
{
     This bit must be cleared through the shadow memory or we won't be
     able to check if automatic setreset is enabled using isAutomaticSetResetEnabled()
...
bool SFE_MMC5983MAisAutomaticSetResetEnabled()
{
     Get the bit value from the shadow register since the IC does not
     allow reading INT_CTRL_0_REG register.
...
bool SFE_MMC5983MAenableXChannel()
{
     This bit must be cleared through the shadow memory or we won't be
...
bool SFE_MMC5983MAdisableXChannel()
{
     This bit must be set through the shadow memory or we won't be
     able to check if the channel is enabled using isXChannelEnabled()
...
bool SFE_MMC5983MAisXChannelEnabled()
{
     Get the bit value from the shadow register since the IC does not
...
bool SFE_MMC5983MAenableYZChannels()
{
     This bit must be cleared through the shadow memory or we won't be
...
bool SFE_MMC5983MAdisableYZChannels()
{
     This bit must be set through the shadow memory or we won't be
     able to check if channels are enabled using areYZChannelsEnabled()
     and since it's a inhibit bit it must be cleared so X channel will
...
bool SFE_MMC5983MAareYZChannelsEnabled()
{
     Get the bit value from the shadow register since the IC does not
     allow reading INT_CTRL_1_REG register.
...
bool SFE_MMC5983MAsetFilterBandwidth(uint16_t bandwidth)
{
     These must be setcleared using the shadow memory since it can be read
    ...
    default
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORINVALID_FILTER_BANDWIDTH);
        success = false;
    }
    break;
    }
...
uint16_t SFE_MMC5983MAgetFilterBandwidth()
{
...
uint16_t SFE_MMC5983MAgetTimeout()
{
     It is rare but there are some devices and some circumstances where the code can become
     ...
     A solution is to timeout after 4  the measurement time (defined by BW10).
    uint16_t timeOut = getFilterBandwidth();  Read the bandwidth (100200400800Hz) from shadow
    timeOut = 800  timeOut;  Convert timeOut to 8421ms
    timeOut = 4;  Convert bw to 321684ms
    timeOut += 1;  Add 1 just in case (for 800Hz)
    return timeOut;
}

bool SFE_MMC5983MAenableContinuousMode()
{
     This bit must be set through the shadow memory or we won't be
...
bool SFE_MMC5983MAdisableContinuousMode()
{
     This bit must be cleared through the shadow memory or we won't be
     able to check if continuous mode is enabled using isContinuousModeEnabled()
...
bool SFE_MMC5983MAisContinuousModeEnabled()
{
     Get the bit value from the shadow register since the IC does not
...
bool SFE_MMC5983MAsetContinuousModeFrequency(uint16_t frequency)
{
     These must be setcleared using the shadow memory since it can be read
    ...
    default
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORINVALID_CONTINUOUS_FREQUENCY);
        success = false;
    }
    break;
    }
...
uint16_t SFE_MMC5983MAgetContinuousModeFrequency()
{
...
bool SFE_MMC5983MAenablePeriodicSet()
{
     This bit must be set through the shadow memory or we won't be
...
bool SFE_MMC5983MAdisablePeriodicSet()
{
     This bit must be cleared through the shadow memory or we won't be
     able to check if periodic set is enabled using isContinuousModeEnabled()
...
bool SFE_MMC5983MAisPeriodicSetEnabled()
{
     Get the bit value from the shadow register since the IC does not
     allow reading INT_CTRL_2_REG register.
...
bool SFE_MMC5983MAsetPeriodicSetSamples(const uint16_t numberOfSamples)
{
    ...
    default
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORINVALID_PERIODIC_SAMPLES);
        success = false;
    }
    break;
    }
...
uint16_t SFE_MMC5983MAgetPeriodicSetSamples()
{
...
bool SFE_MMC5983MAapplyExtraCurrentPosToNeg()
{
     This bit must be set through the shadow memory or we won't be
...
bool SFE_MMC5983MAremoveExtraCurrentPosToNeg()
{
     This bit must be cleared through the shadow memory or we won't be
     able to check if extra current is applied using isExtraCurrentAppliedPosToNeg()
...
bool SFE_MMC5983MAisExtraCurrentAppliedPosToNeg()
{
     Get the bit value from the shadow register since the IC does not
     allow reading INT_CTRL_3_REG register.
...
bool SFE_MMC5983MAapplyExtracurrentNegToPos()
{
     This bit must be set through the shadow memory or we won't be
...
bool SFE_MMC5983MAremoveExtracurrentNegToPos()
{
     This bit must be cleared through the shadow memory or we won't be
     able to check if extra current is applied using isExtraCurrentAppliedNegToPos()
...
bool SFE_MMC5983MAisExtraCurrentAppliedNegToPos()
{
     Get the bit value from the shadow register since the IC does not
     allow reading INT_CTRL_3_REG register.
...
uint32_t SFE_MMC5983MAgetMeasurementX()
{
    ...
    result = buffer[0];  out[1710]
    result = (result  8)  buffer[1];  out[92]
    result = (result  2)  (buffer2bit  6);  out[10]

    return result;
}

uint32_t SFE_MMC5983MAgetMeasurementY()
{
     Set the TM_M bit to start the measurement.
     Do this using the shadow register. If we do it with setRegisterBit
     (read-modify-write) we end up setting the Auto_SR_en bit too as that
    ...
    result = buffer[0];  out[1710]
    result = (result  8)  buffer[1];  out[92]
    result = (result  2)  ((buffer2bit  4) & 0x03);  out[10]

    return result;
}

uint32_t SFE_MMC5983MAgetMeasurementZ()
{
     Set the TM_M bit to start the measurement.
     Do this using the shadow register. If we do it with setRegisterBit
     (read-modify-write) we end up setting the Auto_SR_en bit too as that
    ...
    result = buffer[0];  out[1710]
    result = (result  8)  buffer[1];  out[92]
    result = (result  2)  ((buffer[2]  2) & 0x03);  out[10]

    return result;
}

bool SFE_MMC5983MAgetMeasurementXYZ(uint32_t x, uint32_t y, uint32_t z)
{
     Set the TM_M bit to start the measurement.
     Do this using the shadow register. If we do it with setRegisterBit
     (read-modify-write) we end up setting the Auto_SR_en bit too as that
...
bool SFE_MMC5983MAreadFieldsXYZ(uint32_t x, uint32_t y, uint32_t z)
{
    ...
    if (success)
    {
        x = registerValues[0];  Xout[1710]
        x = (x  8)  registerValues[1];  Xout[92]
        x = (x  2)  (registerValues[6]  6);  Xout[10]
        y = registerValues[2];  Yout[1710]
        y = (y  8)  registerValues[3];  Yout[92]
        y = (y  2)  ((registerValues[6]  4) & 0x03);  Yout[10]
        z = registerValues[4];  Zout[1710]
        z = (z  8)  registerValues[5];  Zout[92]
        z = (z  2)  ((registerValues[6]  2) & 0x03);  Zout[10]
    }
    else
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERRORBUS_ERROR);
...
bool SFE_MMC5983MAclearMeasDoneInterrupt(uint8_t measMask)
{
